...
def transform_df(df):
    ...
    # Group by 'Strain ID' and 'Protein Type', then aggregate 'Binding Markers'
    transformed = df.groupby(['Strain ID', 'Protein Type']).agg({
        'Binding Markers': lambda x: ','.join(x.astype(str))
    }).reset_index()

    # Calculate the count of 'Binding Markers'
    transformed['Number of Binding Markers'] = transformed['Binding Markers'].apply(
...
def explode_markers(df, input_marker_path, output_directory, prefix):
    ...
    # Group and sum the 'Number of Binding markers' by 'Strain ID'
    grouped = df.groupby('Strain ID')['Number of Binding Markers'].sum()
    ...
    # Explode the 'Binding markers' column from a comma-separated string into a list
    df['Binding Markers'] = df['Binding Markers'].str.split(',')

    # Expand the list into a new DataFrame and merge it back
    df = df.explode('Binding Markers')
    ...
    df['marker_Protein'] = df['Binding Markers'] + '_' + df['Protein Type']
    ...
    df_label.drop(labels = ['Binding Markers', 'Number of Binding Markers', 'Protein Type'], axis = 1,
                  inplace = True)
...
def predict_new_data(input_marker_path, model_path, threshold = 0.5, output_directory = ".",
                     prefix = ""):
    """
    ...
    # Calculate probability for α2-6 (assuming class 1 is α2-6)
    a26_probabilities = [round(prob, 3) for prob in new_data_proba]  # 直接使用一维数组
    a23_probabilities = [round(1 - prob, 3) for prob in new_data_proba]
    ...
    for i in range(len(new_data_proba)):
        ...
        a26_prob = a26_probabilities[i]
        a23_prob = a23_probabilities[i]

        # Apply threshold to determine the host
        # If α2-6 probability is greater than or equal to threshold, assign α2-6, otherwise α2-3
        if a26_prob >= threshold:
            host = 'α2-6'
            prob = a26_prob
        else:
            host = 'α2-3'
